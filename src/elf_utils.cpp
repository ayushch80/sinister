...
ELF::ELF(Buffer buffer) : elf(buffer) {}

bool ELF::isStrippedELF()
{
    int shstrndx = read_le_address(elf.buffer, 0x30, 2);
    return (shstrndx == 0);
}
...
std::string ELF::getMagic()
{
    ...
    for (int i = 0; i < 16; i++)
    {
        magic += formatString("%.2x ", elf.buffer[i]);
    }

    return magic;
}

std::string ELF::getClass()
{
    std::string className = (elf.buffer[4] == 1) ? "32-bit" : "64-bit";
    return className;
}

std::string ELF::getData()
{
    std::string dataName = (elf.buffer[5] == 1) ? "Little-endian" : "Big-endian";
    return dataName;
}

std::string ELF::getVersion()
{
    std::string version = formatString("%d", elf.buffer[6]);
    return version;
}

std::string ELF::getOSABI()
{
    int osabi = elf.buffer[7];
...
std::string ELF::getABIVersion()
{
    std::string abiversion = formatString("%d", elf.buffer[8]);
    return abiversion;
}

std::string ELF::getType()
{
    int type = read_le_address(elf.buffer, 0x10, 2);
    std::string typeName = "";
...
std::string ELF::getMachine()
{
    int value = read_le_address(elf.buffer, 0x12, 2);
    std::string isa = "";
...
std::string ELF::getEntrypoint()
{
    int entrypoint = read_le_address(elf.buffer, 0x18, 8);
    std::string entrypointStr = formatString("0x%lx", (long)entrypoint);
    return entrypointStr;
}

std::string ELF::getPHOffset()
{
    std::string offset = formatString("0x%lx", read_le_address(elf.buffer, 0x20, 8));
    return offset;
}

std::string ELF::getSHOffest()
{
    std::string offset = formatString("0x%lx", read_le_address(elf.buffer, 0x28, 8));
    return offset;
}